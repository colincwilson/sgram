...
def sgram(filename,
    ...
    mode = 'magnitude'
    window = signal.windows.blackmanharris(nperseg)
    ...
    if (orig_sf == sf):  # resample to 'sf' samples per second
        ...
        new_size = int(
            len(raw) *
            resample_ratio)  # how many samples in downsampled version?
        x = signal.resample(raw,
                            new_size)  # now sampled at desired sampling freq
    ...
    x2 = np.rint(32000 * (x2[i1:i2] / max(x2[i1:i2]))).astype(
        np.intc)  # scale the signal chunk

    # ----------- compute the spectrogram ---------------------------------
    f, ts, Sxx = signal.spectrogram(x2,
                                    fs=sf,
                                    noverlap=noverlap,
                                    window=window,
                                    nperseg=nperseg,
    ...
    if slice_time > 0:  # if spectral slice is desired, add an axes for it
        ...
        gs = fig.add_gridspec(nrows=1,
                              ncols=2,
                              width_ratios=[figwidth / slice_width, 1])
    ...
    extent = (min(ts), max(ts), min(f), max(f)
    ...
    if slice_time > 0:  # if spectral slice is desired, plot the spectrum
        i = np.argmin(
            np.abs(ts - slice_time))  # find the index of the spectral slice
        ...
        ax2.set_ymargin(
            0)  # put y-axis at bottom and top of axis (as in spectrogram)
        ax2.tick_params(
            labelleft=False)  # do not write the frequency axis labels
